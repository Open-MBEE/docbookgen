  ...
  <fo:block xsl:use-attribute-sets="normal.para.spacing">
    <xsl:if test="contains(@role, 'padding-left-')">
      <xsl:attribute name="margin-left">
        <xsl:choose>
          <xsl:when test="contains(substring-after(@role, 'padding-left-'), ';')">
            <xsl:value-of select="substring-before(substring-after(@role, 'padding-left-'), ';')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="substring-after(@role, 'padding-left-')"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
    </xsl:if>
    <xsl:if test="$keep.together != ''">
      <xsl:attribute name="keep-together.within-column">
        <xsl:value-of select="$keep.together"/></xsl:attribute>
    </xsl:if>
    <xsl:call-template name="anchor"/>
    <xsl:apply-templates/>
  </fo:block>
