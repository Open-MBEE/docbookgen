...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="generate.toc">
        <xsl:choose>
            ...
            <!--xsl:otherwise> appendix toc,title article/appendix nop article toc,title book
                toc,title,figure,table,example,equation chapter toc,title part toc,title preface
                toc,title qandadiv toc qandaset toc reference toc,title sect1 toc sect2 toc sect3
                toc sect4 toc sect5 toc section toc set toc,title </xsl:otherwise
            -->
    ...
    <xsl:template name="user.pagemasters">
        ...
        <fo:page-sequence-master master-name="jpl.title">
            <fo:repeatable-page-master-alternatives>
                ...
                <fo:conditional-page-master-reference master-reference="jpl.title-first"
                    page-position="first"/>
                <fo:conditional-page-master-reference master-reference="jpl.title-odd"
                    odd-or-even="odd"/>
                <fo:conditional-page-master-reference master-reference="jpl.title-even"
                    odd-or-even="even"/>
            </fo:repeatable-page-master-alternatives>
        </fo:page-sequence-master>

        <fo:simple-page-master master-name="jpl.title-first" page-width="{$page.width}"
            ...
            <fo:region-after region-name="xsl-region-after-first" extent="1in"
                display-align="before"/>
        </fo:simple-page-master>

        <fo:simple-page-master master-name="jpl.title-even" page-width="{$page.width}"
            page-height="{$page.height}" margin-top="{$page.margin.top}"
            margin-bottom="{$page.margin.bottom}" margin-left="{$page.margin.inner}"
            margin-right="{$page.margin.outer}">
            <fo:region-body margin-bottom="{$body.margin.bottom}" margin-top="{$body.margin.top}"
            ...
            <fo:region-before region-name="xsl-region-before-even" extent="{$region.before.extent}"
                display-align="before"/>
            <fo:region-after region-name="xsl-region-after-even" extent="1in" display-align="before"
            />
        </fo:simple-page-master>

        <fo:simple-page-master master-name="jpl.title-odd" page-width="{$page.width}"
            page-height="{$page.height}" margin-top="{$page.margin.top}"
            margin-bottom="{$page.margin.bottom}" margin-left="{$page.margin.inner}"
            margin-right="{$page.margin.outer}">
            <fo:region-body margin-bottom="{$body.margin.bottom}" margin-top="{$body.margin.top}"
            ...
            <fo:region-before region-name="xsl-region-before-odd" extent="{$region.before.extent}"
                display-align="before"/>
            <fo:region-after region-name="xsl-region-after-odd" extent="1in" display-align="before"
            />
        </fo:simple-page-master>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$pageclass != 'jpl.title' and $position='center'">
                <xsl:if test="$jpl.header!=''">
                    <fo:block>
                        <xsl:value-of select="$jpl.header"/>
                    </fo:block>
                </xsl:if>
                <xsl:if test="$jpl.subheader">
                    <fo:block>
                        <xsl:value-of select="$jpl.subheader"/>
                    </fo:block>
                </xsl:if>
            </xsl:when>
        ...
        <xsl:choose>
            <xsl:when test="$position='center' and $pageclass != 'jpl.title'">
                ...
                <fo:block font-size="7pt" font-style="italic">
                    <xsl:value-of select="$jpl.subfooter"/>
                </fo:block>
                <fo:block font-size="9pt" font-style="italic">
                    <xsl:value-of select="$jpl.footer"/>
                </fo:block>
            </xsl:when>
            <xsl:when
                test="$pageclass = 'jpl.title' and $sequence = 'first' and not(d:info/d:cover)">
                <fo:block>
                ...
                <fo:block text-align="left" color="gray">
                    <xsl:value-of select="$JPL.logo.text1"/>
                </fo:block>
                <fo:block font-size="10pt" text-align="left" font-style="italic" color="gray"
                    space-after="0.5in">
                    <xsl:value-of select="$JPL.logo.text2"/>
                </fo:block>
            </xsl:when>
    ...
    <xsl:template name="foot.sep.rule">
        ...
        <xsl:if test="$footer.rule !=0">
            <xsl:choose>
                <xsl:when test="$pageclass= 'jpl.title' and $sequence ='first'"> </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template name="book.titlepage.separator">
        <xsl:if test="d:info/d:keywordset">
            ...
            <fo:block break-after="page" text-align="center" font-weight="bold"> THIS PAGE
    ...
    <xsl:template name="book.titlepage.recto">
        ...
        <fo:block-container absolute-position="absolute" top="6.4in" left="0in">
            <fo:block text-align="left" font-size="11pt"> Paper copies of this document may not be
                current and should not be relied on for official purposes. The current version is
                available from a link in the <xsl:value-of
                    select="d:info/d:publisher/d:publishername"/> DocuShare: </fo:block>
            <xsl:if test="d:info/d:publisher/d:address">
                <fo:block text-align="left" font-size="11pt" space-after="0.25in" color="blue"
    ...
    <xsl:template name="book.titlepage.before.verso">
        <xsl:if
            test="d:info/d:author or d:info/d:editor or d:info/d:othercredit or d:info/d:authorgroup/d:editor">
    ...
    <xsl:template name="book.titlepage.verso">
        <xsl:if test="d:info/d:revhistory">
            <fo:block text-align="center" font-weight="bold" space-before="0.25in"
                space-after="0.25in">Change Log</fo:block>
            <fo:table table-layout="fixed" border-width="0.5mm" border-style="solid">
                ...
                <fo:table-body>
                    <fo:table-row background-color="grey">
                        <fo:table-cell>
                            <fo:block font-weight="bold" text-align="center"> Version </fo:block>
                        </fo:table-cell>
                        <fo:table-cell>
                            <fo:block font-weight="bold" text-align="center"> Date </fo:block>
                        </fo:table-cell>
                        <fo:table-cell>
                            <fo:block font-weight="bold" text-align="center"> Sections Changed
                            </fo:block>
                        </fo:table-cell>
                        <fo:table-cell>
                            <fo:block font-weight="bold" text-align="center"> Author </fo:block>
                        </fo:table-cell>
                    </fo:table-row>
        ...
        <xsl:if test="d:info/d:address">
            <fo:block text-align="center" font-weight="bold" space-before="0.25in"
                space-after="0.25in">Distribution List</fo:block>
            ...
            <fo:table>
                ...
                <fo:table-body>
                    <fo:table-row>
                        <fo:table-cell>
                            <fo:block>
                                <xsl:if test="d:personname/d:othername"> ELECTRONIC SIGNATURE ON
                                    FILE </xsl:if>
                            </fo:block>
                            <fo:block> ________________________________________________ </fo:block>
                        </fo:table-cell>
                        <fo:table-cell>
                            <fo:block>
